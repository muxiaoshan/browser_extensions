...
namespace DiagnoseAssistant1.PdfUtils
{
    ...
    public static class PdfImageExtractor
    {
        ...
        /// </summary>
        /// <param name="fileUri">网络文件路径</param>
        /// <returns></returns>
        public static Dictionary<string, System.Drawing.Image> ExtractImages(Uri fileUri)
        {
            var images = new Dictionary<string, System.Drawing.Image>();
            using (var reader = new PdfReader(fileUri))
            {
                var parser = new PdfReaderContentParser(reader);
                ImageRenderListener listener = null;
                for (var i = 1; i <= reader.NumberOfPages; i++)
                {
                    ...
                    if (listener.Images.Count > 0)
                    {
                        ...
                        {
                            images.Add(string.Format("{0}_Page_{1}_Image_{2}{3}",
                                Path.GetFileNameWithoutExtension(fileUri.ToString()), i.ToString("D4"), index.ToString("D4"), pair.Value), pair.Key);
                            index++;
                        }
                    }
                }
